...
| Area | Decision |
|---|---|
| LLM | **Provider-agnostic chain, configured by `LLM_CHAIN`** (`provider:model,...`, per-task overrides `LLM_CHAIN_SQL/_ROUTER/_REWRITE/_ANSWER/_TITLE`). **Default: `openrouter:nvidia/nemotron-3-super-120b-a12b:free` → `openrouter:nex-agi/nex-n2.5-mini:free` → `anthropic:claude-sonnet-5` (paid last resort)**, same chain for side tasks. The router retries once, then falls back; every attempt is traced. Adapters: one OpenAI-compatible adapter (OpenRouter, and equally Cerebras/Groq/Gemini-compat/vLLM) + the Anthropic SDK adapter. History: the first choice was Nemotron → Qwen 3.8 27B (free), but live tests on 2026-09-24 got persistent upstream 429s from Qwen (and Gemma), so the fallback wouldn't have caught a Nemotron failure. The user chose free → free → Claude so the live URL survives free-pool congestion during grading. Account has the 1,000/day free-model quota. |
